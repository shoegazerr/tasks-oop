...
public class Task_4 {
    ...
    public static String noYelling(String str){
        String str1 = "";
        boolean znak = !(str.charAt(str.length()-1) > 32 && str.charAt(str.length()-1) < 64);
    ...
    public static boolean checkLetter(char letter){ ///для xProunce
    ...
    public static String xProunce (String str){
        ...
        for (int i = 1; i< str.length()-1; i++){
            if(str.charAt(i) == 'x'){//если под индексом у нас х
                if(str.charAt(i-1) == ' ' && checkLetter(str.charAt(i+1)) ){ //если предыдущий пробел и след элемент буква
                    str1 += "z"; //чекаем если слово начинается на х
                }
                else if (str.charAt(i-1) == ' ' && str.charAt(i+1) == ' ') //если это просто одиночный х
                    str1 += "ekcs";
                else
                    str1 += "cks";
            } else
        ...
        int d = str.length()-1; //последний
        if (str.charAt(d) == 'x' || str.charAt(d) == 'X'){
            if (checkLetter(str.charAt(d-1))){ // предпоследняя буква то
                str1 += "cks";
            }
            else
                str1 += "ecks";
   ...
   public static String largestGap (int mas[]){
    ...
    public static int revers(int number){
        ...
        int i = 0;
        int number0 = number;
        while (number > 0){ //записывавем в массив поразрядно цифры
            mas[i] = number % 10;
            i++;
            number /= 10;
        }
        Arrays.sort(mas);
        int number2 = 0;
        for (i = 0; i< mas.length; i++){ //записываем поразрядно из массива в number2
            number2 = number2 * 10 + mas[i];
        }
//разница между начальным массивом и отсортированным
    ...
    public static String memeSum(int number1, int number2){
        String str = "";
    ...
    public static void main(String[] args){
        ...
        System.out.println(xProunce("Xbox Inside the box was a xylophonex"));
        System.out.println(largestGap(new int [] {9, 4, 26, 26, 0, 0, 5, 20, 6, 25, 5}));
        System.out.println(revers(7977));
