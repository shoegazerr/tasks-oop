...
public class Task_5 {
    ...
    public static String[] options(String str) {
        ...
        if (letter == 'H'){ charArr = new char[] {'A', 'G'};
        }
        if (letter == 'C'){ charArr = new char [] {'B', 'D'};
        }
        if (letter == 'A'){ charArr = new char [] {'H', 'B'};
        }
        if (letter == 'B'){ charArr = new char [] {'A', 'C'};
        }
        if (letter == 'D'){ charArr = new char [] {'C', 'E'};
        }
        if (letter == 'E'){ charArr = new char [] {'D', 'F'};
        }
        if (letter == 'F'){ charArr = new char [] {'E', 'G'};
        }
        if (letter == 'G'){ charArr = new char [] {'F', 'H'};
        }
    ...
    public static double[] convertCoord (String str){
        ...
        else if (letter == 'E'){ return new double [] {-number, 0};
        }
        else if (letter == 'B'){ return new double[] {number,number};
        }
        else if (letter == 'F'){ return new double[] {-number,-number};
        }
        else if (letter == 'C'){ return new double [] {0, number};
        }
        else if (letter == 'G'){ return new double [] {0, -number};
        }
        else if (letter == 'H'){ return new double[] {-number,number};
        }
        else if (letter == 'D'){ return new double[] {-number,number};
        }
    ...
    public static int totalPoints(String[] results, String str){
        ...
        for (int i = 0; i<str.length(); i++){
            char letter = str.charAt(i);
            if (map.keySet().contains(letter))
                map.put(letter, map.get(letter) + 1); //получаем значение по ключу
            else
                map.put(letter, 1);
        }
    ...
    public static String timeDifference(String inpCity, String inpDate, String outCIty){
        String[] monthes_nazvanie = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
        int[] monthes_chisla = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
        Map<String, Integer[]> monthes = new HashMap<String, Integer[]>();

        int dayCounter = 0;
        for (int i = 0; i < monthes_nazvanie.length; i++){
            monthes.put(monthes_nazvanie[i], new Integer[] {i, dayCounter} );
            //ключ(типо индекс) - название месяца; элемент - кол-во дней //так ведь???
            dayCounter += monthes_chisla[i];  //подсчет дней в году
        }
        int minutesAtYear = dayCounter * 24 * 60; //считаем солько минут в году

        String[] cities_nazvanie= {"Los Angeles", "New York", "Caracas", "Buenos Aires", "London", "Rome", "Moscow", "Tehran", "New Delhi", "Beijing", "Canberra"};
        double[] cities_vremya = {-8, -5, -4.5, -3, 0, 1, 3, 3.5, 5.5, 8, 10};
        Map<String, Double> cities= new HashMap<String, Double>();

        for (int i = 0; i < cities_nazvanie.length; i++){
            cities.put(cities_nazvanie[i], cities_vremya[i]);
        } //ключ(типо индекс) - название города; элемент - время

// перевод даты в минуты
        int totalMinutes = 0;
        String[] date = inpDate.split(" "); //parseInt числа в строке преобразуются в эквивалентные им числовые значения типа int.
        totalMinutes += ( monthes.get(date[0]/*апрель*/)[1] + Integer.parseInt(date[1].replace(",", "")/*  1  */) ) * 24 * 60;
...
// начало счета новой даты
        int monthCounter = 0, mDaysSum = 0;
        for (int i = 0; i < monthes_chisla.length; i++){
            mDaysSum += monthes_chisla[i];
            monthCounter++;
        ...
        res += monthCounter + "-"; // добавление месяца

        totalMinutes -= mDaysSum * 24 * 60;

        res += totalMinutes / (24 * 60); // дня

        totalMinutes -= ( (int)totalMinutes / (24 * 60) ) * 24 * 60;

        res += " " + ((totalMinutes / 60 < 10) ? "0" : "") + totalMinutes / 60; // часов

        totalMinutes -= ( (int)(totalMinutes / 60) ) * 60;

        res += ":" + totalMinutes; // минут
    ...
    if (notNulls<2){  //если это число 100 например
       return "true";
    ...
    return " ";
        ...
        System.out.println(sameLetterPattern("ABCBA","FVGVF"));
